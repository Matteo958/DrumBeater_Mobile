...
namespace trinus{
	public class TrinusUI : MonoBehaviour {
        ...
        [SerializeField] private GameObject _trinusUICamera = default;
	...
	//			trinusCursor.localPosition = new Vector3 (Input.mousePosition.x - Screen.width / 2, Input.mousePosition.y - Screen.height / 2, trinusCursor.localPosition.z);
		...
		public void openGame(){
            ...
            _trinusUICamera.SetActive(false);
			if (disableTrackingWhenPaused)
        ...
        {
            t.GetComponent<UnityEngine.UI.Slider>().value = val;
...
//			dropdown.AddOptions (presets);
		...
		public void setTheme(TrinusTheme theme){
			if (theme != null) {
				...
				UnityEngine.UI.Slider[] allSliders = transform.GetComponentsInChildren<UnityEngine.UI.Slider> (true);
				foreach (UnityEngine.UI.Slider s in allSliders) {
